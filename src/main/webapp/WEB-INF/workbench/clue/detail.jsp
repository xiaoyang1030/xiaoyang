...
<div style="position: relative; left: 40px; top: -30px;">
    ...
    <div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
        <button type="button" class="btn btn-default" onclick="window.location.href='${pageContext.request.contextPath}/clue/convert?id=${clue.id}';"><span
...
<div style="position: relative; top: 40px; left: 40px;">
    ...
    <c:forEach var="remark" items="${clue.clueRemarks}">
        <!-- 备注1 -->
        <div class="remarkDiv" style="height: 60px;">
            ...
            <div style="position: relative; top: -40px; left: 40px;">
                ...
                <small style="color: gray;">
                        ${clue.createtime} 由${clue.createby}</small>
    ...
    <div style="position: relative; top: 60px; left: 40px;">
        ...
        <div style="position: relative;top: 0px;">
            <table class="table table-hover" style="width: 900px;">
                ...
                <tbody id="shichanghuodong">
                <c:forEach items="${relation}" var="relation">
                    <tr id="tr${relation.id}">
                        <td>${relation.activity.name}</td>
                        <td>${relation.activity.startDate}</td>
                        <td>${relation.activity.endDate}</td>
                        <td>${relation.activity.owner}</td>
                        <td><a href="javascript:void(0);" onclick="onBindguanxi('${relation.id}')"
...
<script>
    function bindguanxi() {
        ...
        if (length == 0) {
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/clue/bindguanxi',
                data: {
                    'ids':ids.join(),
                    'clueid':'${clue.id}'
                },
                ...
                success: function (data) {
                    ...
                    if(data.ok){
                        alert(data.mess);
                        $('#shichanghuodong').text("");
                        var dataList = data.list
                        for (var i = 0; i < dataList.length; i++) {
                            $('#shichanghuodong').append("<tr>\n" +
                                "\t\t\t\t\t\t\t\t\t<td>"+dataList[i].name+"</td>\n" +
                                "\t\t\t\t\t\t\t\t\t<td>"+dataList[i].startDate+"</td>\n" +
                                "\t\t\t\t\t\t\t\t\t<td>"+dataList[i].endDate+"</td>\n" +
                                "\t\t\t\t\t\t\t\t\t<td>"+dataList[i].owner+"</td>\n" +
                                "\t\t\t\t\t\t\t\t\t<td><a onclick=\"unbind('${clue.id}',"+dataList[i].id+","+$(this)+")\" href=\"javascript:void(0)\"  style=\"text-decoration: none;\"><span class=\"glyphicon glyphicon-remove\"></span>解除关联</a></td>\n" +
                                "\t\t\t\t\t\t\t</tr>")
                        }
                    }else {
                        alert(data.mess)
    ...
    $('#activityName').keypress(function (event) {
        if (event.keyCode == 13) {
            $.ajax({
                url: '${pageContext.request.contextPath}/clue/seacrhactivity',
                data: {
    ...
    function onBindguanxi(id) {
        $.ajax({
            url: '${pageContext.request.contextPath}/clue/unbind',
            data: {'id': id},
            type: 'post',
            dataType: 'json',
            success: function (data) {
